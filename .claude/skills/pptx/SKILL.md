...
## Workflow: draft in HTML before building a new deck

When the user asks for a **new** presentation/deck (not editing or reading an existing file), default to this two-step flow instead of generating the `.pptx` directly:

1. **Draft as HTML first.** Build the deck as an HTML Artifact — one section per slide, matching the intended layout, content, and design. LLMs produce noticeably better-looking, better-laid-out output in HTML than when generating `.pptx` directly, so this is the cheaper medium to iterate in.
2. **Get sign-off.** Treat the approved HTML as the design and content source of truth. Don't move to step 3 until the user confirms the HTML draft.
3. **Only then build the real `.pptx`.** Generate it from scratch with `pptxgenjs` (see gotchas below), re-implementing the finalized HTML's content and layout. There is no automated HTML→PPTX converter — this is a manual rebuild guided by the approved HTML, not a literal file conversion. Still run the full QA section below on the result.

Skip step 1 only if the user explicitly asks to go straight to `.pptx`, or the task is editing/reading an existing deck rather than creating a new one.
